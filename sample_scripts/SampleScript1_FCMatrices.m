...
% Add relevant info
GTstruct = GTaddvalue(GTstruct, 'Field', 'ID', 'NewValue', {'01'}, 'Elements', [1,2])
GTstruct = GTaddvalue(GTstruct, 'Field', 'ID', 'NewValue', {'02'}, 'Elements', [3,4])
GTstruct = GTaddvalue(GTstruct, 'Field', 'Condition', 'NewValue', {'RS'})
GTstruct = GTaddvalue(GTstruct,'Field', 'FC_measure', 'NewValue', {'AEC'}, 'Elements', [1, 3])
GTstruct = GTaddvalue(GTstruct, 'Field', 'FC_measure', 'NewValue', {'PLV'}, 'Elements', [2,4])
GTstruct = GTaddvalue(GTstruct, 'Field', 'Freq_Band', 'NewValue', {'alpha'}, 'Elements', [1,3])
GTstruct = GTaddvalue(GTstruct, 'Field', 'Freq_Band', 'NewValue', {'theta'}, 'Elements', [2,4])


% ALTERNATIVE: Add relevant info from file names
GTstruct0 = GTloadBst(file_names, 'conn_mat', 'Field', 'mat_or', 'InDir', files_dir)
GTstruct0_fieldnames = GTgetfields_FileName(file_names, 'Fields', 'ID_Condtion_FCMeasure_FreqBand')
GTstruct0 = GTmerge(GTstruct0, GTstruct0_fieldnames)
...
GTstruct_AllNetworks = GTNodeSel(GTstruct, ...
    ...
    'Field', 'mat_or', ...
    'OtherFields', {'ID', 'Condition', 'FC_measure','Freq_Band'})
...
% Plot matrices
GTimagesc(GTstruct_AllNetworks, 'DataField', 'mat_or', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)
GTimagesc(GTstruct_AllNetworks, 'DataField', 'mat_thresh', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)
GTimagesc(GTstruct_AllNetworks, 'DataField', 'mat_bin', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)


% Select only the nodes relative to one DMN
GTstruct_DMN = GTNodeSel(GTstruct, ...
    'Nodes', atlas_scouts, ...
    'SelNodes',DefaultModeNetwork, ...
    'Field', 'mat_or', ...
    'OtherFields', {'ID', 'Condition', 'FC_measure','Freq_Band'})
GTstruct_DMN = GTthreshold(GTstruct_DMN,...
    'Field', 'mat_or', ...
    'Perc', 90, ...
    'NewField', 'mat_thresh')
GTstruct_DMN = GTbinarize(GTstruct_DMN, ...
    'Field', 'mat_thresh', ...
    'NewField', 'mat_bin')

GTimagesc(GTstruct_DMN, 'DataField', 'mat_or', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)
GTimagesc(GTstruct_DMN, 'DataField', 'mat_thresh', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)
GTimagesc(GTstruct_DMN, 'DataField', 'mat_bin', 'LabelFields', {'ID', 'FC_measure', 'Freq_Band'}, 'Ncols', 2)
